...
char getTetrisKey(TetrisState state, bool fromUser, bool toFile) {
  ...
  if (fromUser == true) {
    ...
    if (infStream.eof() == true)
      ...
      infStream.get(key); // why not "infStream >> key" ? // 스페이스 받을려고 하는거 
  ...
  if (toFile == true) {
    ...
    // 요구사항 2) 
    TenBuffer.push_back(key);
    if (TenBuffer.length() >= 10){
      outfStream << TenBuffer.c_str();
      Test << TenBuffer.c_str() << endl;
      TenBuffer.clear();
    }
